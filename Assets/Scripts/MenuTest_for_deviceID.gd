...
# Faktiska bilscener (PackedScene)
var car_scenes = [
...
func _unhandled_input(event):
	...
	if device >= player_count:
		return
	
	var pdata = player_data[device]
	if pdata["selection_locked"]:
		if event.is_action_pressed("menu_back"):
			pdata["selection_locked"] = false
			_update_label(device)
			_check_all_selected()
		return
	...
	if event.is_action_pressed("menu_right"):
		if pdata["menu_index"] == 0:
			...
			pdata["camera_cull_mask"] = pdata["car"]
			for i in range(car_scenes.size()):
				car_preview[pdata["device_id"]].set_cull_mask_value(i+1, 1 if i == pdata["camera_cull_mask"] else 0)
		else:
		...
		if pdata["menu_index"] == 0:
			...
			pdata["camera_cull_mask"] = pdata["car"]
			for i in range(car_scenes.size()):
				car_preview[pdata["device_id"]].set_cull_mask_value(i + 1, 1 if i == pdata["car"] else 0)
		else:
...
func _update_label(player_index):
	...
	var text = "Spelare %d - Bilval: %s | Hjulval: %s [%s]" % [
		player_index + 1,
		car_names[pdata["car"]],
...
func _check_all_selected():
	...
	for pdata in player_data:
		if !pdata["selection_locked"]:
			all_selected = false
			break
	Level_select.visible = all_selected
	if all_selected:
